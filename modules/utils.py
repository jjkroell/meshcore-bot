...
def _geocode_result_names_city(location: Any, city_clean: str) -> bool:
    """Check that a Nominatim result's own place name matches the queried city.

    A state/country-constrained query (e.g. "London, BC, CA") for a city that
    doesn't actually exist in that state/province can still return a non-empty
    result: Nominatim falls back to its best fuzzy match, which may be an
    unrelated landmark or POI whose name merely contains the query (e.g.
    "Steveston-London Secondary School" in Richmond, BC for a "London, BC, CA"
    search). Comparison ignores case, accents, and a trailing parenthetical
    (e.g. "London (city)").
    """
    ...
    return _norm(name) == _norm(city_clean)
...
def geocode_city_sync(bot: Any, city: str, default_state: Optional[str] = None,
    ...
    try:
        ...
        city_clean = city.strip()
        state_abbr = None
        # Holds a worldwide-search match to fall back to if the default-country-biased
        # queries below don't find anything (set only when we deliberately skip the
        # worldwide match in favor of trying the default country first).
        ...
        if not state_abbr and not country_name:
            ...
            if location:
                ...
                # Prefer a same-name match inside the deployment's default country over a
                # more "globally prominent" match elsewhere (e.g. "London" -> London, ON,
                # not London, England, when default_country=CA). Fall through to try the
                # default country/state queries below, keeping this result as a last-resort
                # fallback in case those don't find anything either.
                if include_address_info and not result_in_default_country:
                    fallback_location = location
                elif result_in_default_country and is_obscure_location:
                    # Fall through to try with default country/state
                    fallback_location = location
                else:
        ...
        if default_state and default_state.strip():
            ...
            if cached_lat and cached_lon:
                ...
                location = rate_limited_nominatim_geocode_sync(bot, cache_query, timeout=timeout)
                if location and _geocode_result_names_city(location, city_clean):
                    bot.db_manager.cache_geocoding(cache_query, location.latitude, location.longitude)
                    lat, lon = location.latitude, location.longitude
                else:
                    lat, lon = None, None
        ...
        # Nothing matched inside the default country - fall back to the original
        # worldwide match (e.g. a genuinely international city with no same-named
        # place in the default country) rather than failing outright.
        if fallback_location is not None:
            bot.db_manager.cache_geocoding(city_clean, fallback_location.latitude, fallback_location.longitude)
            lat, lon = fallback_location.latitude, fallback_location.longitude
