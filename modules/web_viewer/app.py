...
class BotDataViewer:
    ...
    def _setup_routes(self):
        ...
        def login():
            ...
            if request.method == 'POST':
                import hmac, bcrypt as _bcrypt, time as _time
                ...
                _max_attempts = 10
                _attempts = self._login_failures.get(_ip, [])
                ...
                stored = self.web_viewer_password
                if stored.startswith('$2b$') or stored.startswith('$2a$'):
                    password_ok = _bcrypt.checkpw(password.encode(), stored.encode())
                else:
                    password_ok = hmac.compare_digest(password, stored)
                if password_ok:
                    self._login_failures.pop(_ip, None)
                ...
                self._login_failures[_ip] = _attempts
        ...
        def send_message_page():
            """Send a message to a mesh channel via the webhook."""
            channels = [ch['channel_name'] for ch in self._get_channels()
                        if ch.get('channel_name') and ch.get('channel_idx') is not None]
            webhook_enabled = self.config.getboolean('Webhook', 'enabled', fallback=False)
            return render_template('send.html', channels=channels, webhook_enabled=webhook_enabled)

        @self.app.route('/scheduled')
        def scheduled_messages_page():
            """Scheduled messages management page."""
            channels = [ch['channel_name'] for ch in self._get_channels()
                        if ch.get('channel_name') and ch.get('channel_idx') is not None]
            return render_template('scheduled.html', channels=channels)

        @self.app.route('/api/scheduled-messages')
        def api_scheduled_messages_get():
            """Return all scheduled messages from config.ini."""
            import re, configparser
            messages = []
            # Read fresh from disk — configparser.read() merges and never removes
            # deleted keys, so we always open a new instance here.
            fresh = configparser.ConfigParser()
            fresh.read(self.config_path, encoding='utf-8')
            if fresh.has_section('Scheduled_Messages'):
                for key, value in fresh.items('Scheduled_Messages'):
                    try:
                        channel, message = value.split(':', 1)
                    except ValueError:
                        continue
                    if re.fullmatch(r'[0-2][0-9][0-5][0-9]', key):
                        msg_type = 'fixed'
                        label = f"{key[:2]}:{key[2:]}"
                    elif re.fullmatch(r'every_(\d+h)?(\d+m)?', key) and key != 'every_':
                        msg_type = 'interval'
                        label = key.replace('every_', 'Every ').replace('h', 'h ').replace('m', 'm').strip()
                    else:
                        continue
        ...
        def api_scheduled_messages_create():
            """Add a new scheduled message."""
            return self._scheduled_message_write(request.get_json(silent=True) or {}, action='create')

        @self.app.route('/api/scheduled-messages/<path:key>', methods=['PUT'])
        def api_scheduled_messages_update(key):
            """Update an existing scheduled message."""
            data = request.get_json(silent=True) or {}
            data['original_key'] = key
            return self._scheduled_message_write(data, action='update')

        @self.app.route('/api/scheduled-messages/<path:key>', methods=['DELETE'])
        def api_scheduled_messages_delete(key):
            """Delete a scheduled message."""
            return self._scheduled_message_write({'original_key': key}, action='delete')

        @self.app.route('/api/send', methods=['POST'])
        def api_send_message():
            """Proxy a send-message request to the local webhook service."""
            ...
            webhook_host = self.config.get('Webhook', 'host', fallback='127.0.0.1')
            if webhook_host == '0.0.0.0':
                webhook_host = '127.0.0.1'
            secret_token = self.config.get('Webhook', 'secret_token', fallback='')
            payload = json.dumps({'channel': channel, 'message': message}).encode()
    ...
    def _scheduled_message_write(self, data: dict, action: str):
        """Create, update or delete a scheduled message entry in config.ini."""
        import re
        from flask import jsonify as _jsonify

        def valid_key(k):
            return (re.fullmatch(r'[0-2][0-9][0-5][0-9]', k) or
                    (re.fullmatch(r'every_(\d+h)?(\d+m)?', k) and k != 'every_'))
        ...
        if action in ('create', 'update'):
            if not valid_key(key):
                return _jsonify({'error': f"Invalid key '{key}'. Use HHMM or every_Xh / every_XhYm."}), 400
            if not channel or not message:
                return _jsonify({'error': 'channel and message are required'}), 400
        ...
        try:
            ...
            return _jsonify({'error': f'Cannot read config: {e}'}), 500

        # Locate [Scheduled_Messages] section
        section_pat = re.compile(
            r'(\[Scheduled_Messages\])(.*?)(?=\n\[|\Z)',
            re.IGNORECASE | re.DOTALL,
        )
        match = section_pat.search(raw)
        header = match.group(1) if match else '[Scheduled_Messages]'
        body   = match.group(2) if match else ''

        # Parse existing entries (preserve leading comment/blank lines)
        ...
        if action == 'delete':
            if orig_key not in entries:
                return _jsonify({'error': f"Key '{orig_key}' not found"}), 404
            del entries[orig_key]
            order.remove(orig_key)
        elif action == 'update':
            if orig_key not in entries:
                return _jsonify({'error': f"Key '{orig_key}' not found"}), 404
            del entries[orig_key]
            order.remove(orig_key)
            entries[key] = (channel, message)
            order.append(key)
        else:  # create
            if key in entries:
                return _jsonify({'error': f"Key '{key}' already exists — use PUT to update"}), 409
            entries[key] = (channel, message)
            order.append(key)

        # Rebuild the section text
        new_body = '\n' + '\n'.join(extras)
        for k in order:
            ch, msg = entries[k]
            new_body += f'\n{k} = {ch}:{msg}'
        ...
        if match:
            raw = raw[:match.start()] + new_section + raw[match.end():]
        else:
            raw = raw.rstrip() + '\n\n' + new_section + '\n'

        # Write back — only [Scheduled_Messages] content changed
        try:
            with open(self.config_path, 'w') as f:
                f.write(raw)
            self.config.read(self.config_path)
        except Exception as e:
            return _jsonify({'error': f'Cannot write config: {e}'}), 500

        # Signal the bot to hot-reload the scheduler
        try:
            with self._get_db_connection() as conn:
                conn.execute(
                    "INSERT INTO channel_operations (operation_type, status) VALUES ('reload_scheduler', 'pending')"
                )
                conn.commit()
        except Exception as e:
            self.logger.warning('Could not queue reload_scheduler: %s', e)
