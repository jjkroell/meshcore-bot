...
class CommandManager:
    ...
    async def send_channel_message(
        ...
        Args:
            reply_to_name: If set, prefixes the content with "@[name] " so it's
                clear which node the reply is addressed to in the shared channel.
                Skipped if content already mentions that name (some commands
                build their own @[name] tag via a configured response_format).
        """
        if not self.bot.connected or not self.bot.meshcore:
            return False

        if reply_to_name and f"@[{reply_to_name}]" not in content:
            content = f"@[{reply_to_name}] {content}"
    ...
    async def send_channel_messages_chunked(
        ...
        *,
        ...
        Args:
            ...
            reply_to_name: If set, prefixes only the first chunk with "@[name] ".
        ...
        for i, chunk in enumerate(chunks):
            ...
            success = await self.send_channel_message(
                ...
                scope=scope,
                reply_to_name=reply_to_name if i == 0 else None,
            )
            if not success:
                self.logger.warning(
        ...
        try:
            ...
            if message.is_dm:
                ...
                return await self.send_channel_message(
                    ...
                    reply_to_name=message.sender_id,
                )
        except Exception as e:
